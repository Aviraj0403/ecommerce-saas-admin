...
const SOCKET_URL = import.meta.env.VITE_SOCKET_URL || 'http://localhost:6006';
...
class SocketClient {
  ...
  private setupApplicationEvents(): void {
    ...
    // Order update events (admin-specific)
    this.socket.on('order:created', (data) => {
      console.log('New order created:', data);
      toast.success(`New order #${data.orderNumber} received from ${data.customerName}`);
    ...
    this.socket.on('order:cancelled', (data) => {
      ...
      toast.warning(`Order #${data.orderNumber} was cancelled`);
      this.triggerCallbacks('order:cancelled', data);
    });

    // Product events
    this.socket.on('product:low_stock', (data) => {
      console.log('Low stock alert:', data);
      toast.warning(`Low stock alert: ${data.productName} (${data.stock} remaining)`);
      this.triggerCallbacks('product:low_stock', data);
    });

    this.socket.on('product:out_of_stock', (data) => {
      console.log('Out of stock alert:', data);
      toast.error(`Out of stock: ${data.productName}`);
      this.triggerCallbacks('product:out_of_stock', data);
    });

    // Customer events
    this.socket.on('customer:registered', (data) => {
      console.log('New customer registered:', data);
      toast.success(`New customer registered: ${data.customerName}`);
      this.triggerCallbacks('customer:registered', data);
    });

    // System events
    this.socket.on('system:metrics_updated', (data) => {
      console.log('System metrics updated:', data);
      this.triggerCallbacks('system:metrics_updated', data);
    });
