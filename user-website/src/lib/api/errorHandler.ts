...
export function displayAPIError(error: unknown): void {
  ...
  switch (apiError.code) {
    case 'NETWORK_ERROR':
    case 'TIMEOUT_ERROR':
      toast.error(apiError.message, 'Please check your connection and try again.');
      break;

    case 'UNAUTHORIZED':
      toast.error(apiError.message, 'Redirecting to login page...');
      break;

    case 'FORBIDDEN':
      toast.error(apiError.message, 'Contact support if you believe this is an error.');
      break;

    case 'VALIDATION_ERROR':
      toast.error(apiError.message, apiError.field ? `Issue with field: ${apiError.field}` : undefined);
      break;

    case 'RATE_LIMIT':
      toast.warning(apiError.message, 'Please wait before making more requests.');
      break;
    ...
    case 'GATEWAY_TIMEOUT':
      toast.error(apiError.message, 'If the problem persists, please contact support.');
      break;
